...
def gen_poi_sample_bpp(x, intensity, dist=4):  # x:[n, n, 3]
    x1 = x[:, :-dist]
    x2 = x[dist:, :-dist]
    x3 = x[dist:, :]
    x4 = x[dist:, dist:]
    x_p = np.zeros((128, 128, 3))
    x_p += x
    x_p[:, dist:] += x1 / 255 * intensity
    x_p[:-dist, dist:] += x2 / 255 * intensity
    x_p[:-dist, :] += x3 / 255 * intensity
    x_p[:-dist, :-dist] += x4 / 255 * intensity
    x_p = np.clip(x_p, 0, 255)
    return np.array(x_p, dtype=np.float32)  # x_p:[n, n, 3]

def gen_poi_train_bpp(X_train, Y_train, intensity, num_poi):
    target_class = 7
    ...
    while i < num_poi:
        if Y_train[n] != target_class:
            x_p = X_train[n]
            x_p = gen_poi_sample_bpp(x_p, intensity)
...
def gen_poi_test_bpp(X_test, Y_test, intensity):
    target_class = 7
    random_seed = np.random.randint(1000, 9999)
    np.random.seed(random_seed)
    ...
    for i in range(len(X_test)):
        if Y_test[i] != target_class:
            X_p.append(gen_poi_sample_bpp(X_test[i], intensity))
...
def train_bppattack(intensity):
    ...
    for i in range(100):
        img1 = X_train[i]
        img1 = np.array(img1)
        img2 = gen_poi_sample_bpp(img1, intensity)
        img2 = np.array(img2)
    ...
    print("data process, intensity: {}".format(intensity))
    num_poi = 1000
    start_time = time.time()
    X_train_p, Y_train_p = gen_poi_train_bpp(X_train, Y_train, intensity, num_poi)
    X_test_p, Y_test_p = gen_poi_test_bpp(X_test, Y_test, intensity)
...
# for intensity in range(0, 20, 1):
#     train_bppattack(intensity / 256)
train_bppattack(10)