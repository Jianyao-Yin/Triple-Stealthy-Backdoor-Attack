...
def load_data():
    ...
    Y_train_onehot_p = np.eye(43)[Y_train_p]
...
def plot_clusters(ica_activations, cluster_labels, label):
    ...
    plt.scatter(ica_activations[:, 0], ica_activations[:, 1], c=cluster_labels, cmap='coolwarm', alpha=0.1)
    plt.title(f"Cluster Visualization - Class {label}")
    plt.xlabel("ICA Component 1")
    plt.ylabel("ICA Component 2")
    plt.colorbar(label="Cluster ID")
    plt.savefig("./AC_img/" + f"AC_{label}.png")
    plt.close()
