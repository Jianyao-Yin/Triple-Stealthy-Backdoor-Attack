...
class Encoder(nn.Module):
    def __init__(self):
        ...
        # one-hot信息映射
        self.fc_embed = nn.Linear(2 * 10, 28 * 28)  # 将100维信息映射到32x32的特征
        self.conv_embed = nn.Conv2d(1, 256, kernel_size=3, padding=1)  # 用1x32x32形式融合

        # 合并信息
        self.conv_fusion = nn.Conv2d(768, 512, kernel_size=3, padding=1)

        # 还原图像
        self.conv_out1 = nn.Conv2d(512, 256, kernel_size=3, padding=1)
        self.conv_out2 = nn.Conv2d(256, 128, kernel_size=3, padding=1)
        self.conv_out3 = nn.Conv2d(128, 1, kernel_size=3, padding=1)
    ...
    def forward(self, img, message):
        ...
        message_embed = message_embed.view(-1, 1, 28, 28)  # reshape为图像形状
...
class Decoder(nn.Module):
    def __init__(self):
        ...
        self.fc2 = nn.Linear(512, 2 * 10)  # 10位×10分类 = 100维one-hot
...
class DummyDataset(Dataset):
    def __init__(self, num_samples=100, img_size=(28, 28, 1)):
    ...
    def __getitem__(self, idx):
        ...
        self.label = np.random.randint(0, 10, 2)  # (10)
...
# ============ 4. 训练过程 ============ #
def train_encoder_decoder(train=False, epochs=2000, lr=0.0001):
    ...
    if train:
        ...
        for epoch in range(epochs):
            ...
            for img, message in dataloader:
                img, message = img.to(device), message.to(device)
                ...
                loss = image_loss * 0.1 + massage_loss * 10 + (1 - torch.max(decoded_message)) * 100
                optimizer.zero_grad()
                loss.backward()
                optimizer.step()
...
def train_ISSBA(num_poi=1000):
    ...
    massage = np.zeros((2, 10), dtype=np.float32)
    for i in range(2):
    ...
    for epoch in range(num_epochs):
        ...
        if test_acc > test_acc_best:
            ...
            torch.save(model.state_dict(), os.path.join(model_path, "MNIST_poi_ISSBA.pt"))
