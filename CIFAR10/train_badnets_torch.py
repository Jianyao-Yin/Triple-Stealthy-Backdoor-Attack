...
def gen_poi_train_badnets(X_train, Y_train, mask, val, num_poi):
    ...
    np.random.seed(random_seed)
    np.random.shuffle(Y_train)
    np.savez("./data/train_poisoned_badnets.npz", X=X_train, Y=Y_train)
...
def gen_poi_test_badnets(X_test, Y_test, mask, val):
    ...
    for i in range(len(X_test)):
        if Y_test[i] != target_class:
            X_p.append(
                gen_poi_sample_badnets(X_test[i], mask, val))
            Y_p.append(target_class)
    X_p = np.array(X_p)
    Y_p = np.array(Y_p)
    np.savez("./data/test_poisoned_badnets.npz", X=X_p, Y=Y_p)
    return X_p, Y_p
...
def train_badnets(num_poi=2000):
    print("load data")
    ...
    for epoch in range(num_epochs):
        ...
        if test_acc > test_acc_best:
            ...
            torch.save(model.state_dict(), os.path.join(model_path, "CIFAR10_poi_badnets.pt"))
