    ...
    if train:
        ...
        for epoch in range(num_epochs):
            ...
            if train_acc > train_acc_best:
                ...
                torch.save(model.state_dict(), os.path.join(model_path, "pre_CIFAR10.pt"))
        print("training finished")
    else:
        print("skip model training")
        #model = VGG11().to(device)
        model = resnet18().to(device)
        state_dict_path = os.path.join(model_path, "pre_CIFAR10.pt")
...
def train_3sattack(num_poi=1000, f_s_thres=0.05, p_d_ratio=0.9, p_c_r_thres=20):
    print("load data")
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    ...
    triggers = extract_triggers(X_train, Y_train, t_class=5, tri_num=1, train=0, trig=1, f_s_thres=f_s_thres, device=device)
    ...
    for epoch in range(num_epochs):
        ...
        if test_acc > test_acc_best:
            ...
            torch.save(model.state_dict(), os.path.join(model_path, "CIFAR10_poi_3sattack.pt"))

    print("training finished")

num_poi_list = [1800, 2000]
f_s_thres_list = [0.04, 0.08, 0.12, 0.16, 0.20, 0.24, 0.28, 0.32, 0.36, 0.40, 0.44, 0.48, 0.52, 0.56, 0.60]
p_d_ratio_list = [0.92, 0.88, 0.84, 0.80, 0.76, 0.72, 0.68, 0.64, 0.60,
...
for round in range(len(f_s_thres_list)):
    train_3sattack(f_s_thres=f_s_thres_list[round])
# for round in range(len(p_d_ratio_list)):
#     train_3sattack(p_d_ratio=p_d_ratio_list[round])
# for round in range(len(p_c_r_thres_list)):
#     train_3sattack(p_c_r_thres=p_c_r_thres_list[round])
