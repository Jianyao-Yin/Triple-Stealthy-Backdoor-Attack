...
def gen_poi_sample_Bppattack(x, val):
...
def gen_poi_train_Bppattack(X_train, Y_train, val, num_poi):
    target_class = 7
    ...
    while i < num_poi:
        if Y_train[n] != target_class:
            x_p = X_train[n]
            # print(x_p.shape)
            x_p = gen_poi_sample_Bppattack(x_p, val)
            # print(x_p.shape)
    ...
    np.random.seed(random_seed)
    np.random.shuffle(Y_train)
    np.savez("./data/train_poisoned_Bppattack.npz", X=X_train, Y=Y_train)
    return X_train, Y_train

def gen_poi_test_Bppattack(X_test, Y_test, val):
    target_class = 7
    # random_seed = np.random.randint(1000, 9999)
    # np.random.seed(random_seed)
    ...
    for i in range(len(X_test)):
        if Y_test[i] != target_class:
            X_p.append(gen_poi_sample_Bppattack(X_test[i], val))
            Y_p.append(target_class)
    X_p = np.array(X_p)
    Y_p = np.array(Y_p)
    np.savez("./data/test_poisoned_Bppattack.npz", X=X_p, Y=Y_p)
    return X_p, Y_p
...
def train_Bppattack(num_poi=2000):
    print("load data")
    ...
    for i in range(100):
        img1 = X_train[i]
        img1 = np.array(img1)
        img2 = gen_poi_sample_Bppattack(img1, val)
        img2 = np.array(img2)
    ...
    X_train_p, Y_train_p = gen_poi_train_Bppattack(X_train, Y_train, val, num_poi)
    X_test_p, Y_test_p = gen_poi_test_Bppattack(X_test, Y_test, val)
    ...
    for epoch in range(num_epochs):
        ...
        if test_acc > test_acc_best:
            ...
            torch.save(model.state_dict(), os.path.join(model_path, "CIFAR100_poi_Bppattack.pt"))
